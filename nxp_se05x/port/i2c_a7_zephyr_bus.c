...
static uint8_t parse_address(const char *connection_string)
{
	if (connection_string == NULL || strcmp(connection_string, "none") == 0) {
		return se05x_zephyr_i2c_default_address();
	}

	const char *addr = strchr(connection_string, ':');

	addr = addr == NULL ? connection_string : addr + 1;
...
i2c_error_t axI2CInit(void **conn_ctx, const char *pDevName)
{
	...
	if (registered_bus != NULL) {
		...
		se05x_zephyr_i2c_config_t config = se05x_zephyr_i2c_default_config();

		config.i2c.addr = conn->address;
		if (se05x_zephyr_i2c_bus_create(&conn->bus, &config) != 0) {
			free(conn);
			return I2C_FAILED;
		}
		conn->owns_bus = true;
		       ...
		       unsigned char *pTx, unsigned short txLen)
{
	...
	if (conn == NULL || pTx == NULL || txLen == 0U || txLen > MAX_DATA_LEN) {
		return I2C_FAILED;
	}
		      ...
		      unsigned char *pRx, unsigned short rxLen)
{
	...
	if (conn == NULL || pRx == NULL || rxLen == 0U || rxLen > MAX_DATA_LEN) {
		return I2C_FAILED;
	}
