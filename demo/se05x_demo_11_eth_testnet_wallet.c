...
LOG_MODULE_REGISTER(se05x_demo_eth_testnet, LOG_LEVEL_INF);
...
/*
 * Demo 11: eth_testnet_wallet.
 *
 * 这个 demo 把 Demo09/Demo10 已经验证过的 secp256k1 和 ETH legacy
 * signing 链路推进到真实 Sepolia 测试网可用的形态。核心差异是：
 * Demo11 使用 SE05x persistent object 0xEF110001 保存钱包私钥，所以地址稳定；
 * Demo10 使用 transient 测试 key，所以每次运行都可能产生新地址。
 *
 * 真实 ETH legacy 转账需要以下字段：
 *   1. nonce：发送地址已经发出的交易序号，必须从链上查询。
 *   2. gasPrice：legacy 交易 gas 单价，单位 wei。
 *   3. gasLimit：普通 ETH 转账通常是 21000。
 *   4. to：20 字节收款地址。
 *   5. value：转账金额，单位 wei。
 *   6. data：普通 ETH 转账为空；合约调用时是 calldata。
 *   7. chainId：EIP-155 防重放链 ID，Sepolia 是 11155111。
 *
 * 签名流程：
 *   1. nRF 将交易字段按 legacy/EIP-155 规则做 RLP 编码：
 *      [nonce, gasPrice, gasLimit, to, value, data, chainId, 0, 0]
 *   2. nRF 对 RLP signing payload 计算 Ethereum Keccak-256。
 *      注意 Ethereum 使用 Keccak-256，不是 FIPS SHA3-256。
 *   3. SE05x 创建或复用 persistent secp256k1 私钥对象 0xEF110001。
 *   4. nRF 读取公钥部分，用 Keccak-256(pubkey X||Y) 推导 ETH 地址。
 *   5. SE05x 对第 2 步 digest 执行 ECDSA sign_digest，私钥不导出。
 *   6. nRF 解析 DER 得到 r/s，并做 low-S 归一化。
 *   7. 因 SE05x ECDSA 接口不返回 recovery id，固件输出两个 v/raw tx
 *      候选；PC/手机通过恢复地址选择正确 v 后再广播。
 *
 * 安全边界：
 *   - secp256k1 未启用时会调用 CreateCurve 写一次曲线参数 NVM。
 *   - 钱包私钥对象 0xEF110001 会持久保存在 SE05x 内，掉电后仍存在。
 *   - 只有明确输入 AT+X=DELETE_TESTNET_KEY 才会删除 0xEF110001。
 *   - 生产钱包还需要用户确认、屏幕显示、PIN、交易解析、防回滚、
 *     备份恢复和通信认证；本 demo 只验证测试网签名主链路。
 *
 * 运行时串口只输出英文 ASCII，避免串口工具中文编码不一致造成乱码。
 */
...
{
	printk("\n========== SE05x Demo 11: ETH testnet wallet ==========\n");
	printk("AT+H : Show this menu\n");
	printk("AT+A : Create/reuse persistent wallet key and print stable ETH address\n");
	printk("AT+P : Print current legacy transaction fields\n");
	printk("AT+R : Reset fields to Sepolia sample values\n");
	printk("AT+S : Sign current ETH Sepolia legacy transaction fields\n");
	...
	printk("AT+X=DELETE_TESTNET_KEY : Delete persistent Demo11 wallet key\n");
	printk("AT+Q : Quit Demo 11 and close SE05x session\n");
	printk("Private key policy: persistent in SE05x object 0xEF110001, never exported.\n");
	printk("ETH note: SE05x returns ECDSA DER only; recovery id is selected by PC/phone.\n");
	printk("NVM policy: may create secp256k1 curve once and wallet key once.\n");
	printk("====================================================\n\n");
}
...
static void eth_reset_tx_to_sample(eth_legacy_tx_t *tx)
{
	...
	tx->value_wei = 100000000000000ULL;
	tx->data_len = 0U;
	tx->chain_id = 11155111ULL;
}
...
static bool eth_wallet_key_exists(se05x_demo_stats_t *stats,
				  pSe05xSession_t se_session,
				  bool *exists)
{
	SE05x_Result_t result = kSE05x_Result_NA;
	smStatus_t sw = Se05x_API_CheckObjectExists(se_session,
						    SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET,
						    &result);

	if (sw != SM_OK) {
		se05x_demo_mark_fail_sw(stats, "CheckObjectExists(ETH_TESTNET_KEY)", sw);
		return false;
	}
...
static bool eth_delete_wallet_key_if_confirmed(se05x_demo_stats_t *stats,
					       pSe05xSession_t se_session)
{
	bool exists = false;

	if (!eth_wallet_key_exists(stats, se_session, &exists)) {
		return false;
	}

	if (!exists) {
		printk("OK wallet key already absent object_id=0x%08" PRIX32 "\n",
		       (uint32_t)SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
		se05x_demo_mark_pass(stats, "WalletKeyAlreadyAbsent");
		return true;
	}

	LOG_WRN("deleting persistent Demo11 wallet key object_id=0x%08" PRIX32,
		(uint32_t)SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
	smStatus_t sw = Se05x_API_DeleteSecureObject(se_session,
						     SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
	if (sw != SM_OK) {
		se05x_demo_mark_fail_sw(stats, "DeleteSecureObject(ETH_TESTNET_KEY)", sw);
		return false;
	}

	printk("OK deleted persistent wallet key object_id=0x%08" PRIX32 "\n",
	       (uint32_t)SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
	se05x_demo_mark_pass(stats, "DeleteEthTestnetWalletKey");
	return true;
}

static bool eth_prepare_persistent_wallet_key(se05x_demo_stats_t *stats,
					      ...
					      sss_object_t *wallet_key)
{
	...
	if (status != kStatus_SSS_Success) {
		se05x_demo_mark_fail_status(stats, "sss_key_object_init(ETH_TESTNET)", status);
		return false;
	}

	if (exists) {
		status = sss_key_object_get_handle(wallet_key,
						   SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
		if (status != kStatus_SSS_Success) {
			se05x_demo_mark_fail_status(stats, "GetHandle(ETH_TESTNET_KEY)", status);
			return false;
		}

		printk("ETH_WALLET_KEY_OBJECT_ID=0x%08" PRIX32 "\n",
		       (uint32_t)SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
		printk("ETH_WALLET_KEY_CREATED=no\n");
		se05x_demo_mark_pass(stats, "GetHandle(ETH_TESTNET_KEY)");
		return true;
	}

	LOG_WRN("creating persistent Demo11 wallet key object_id=0x%08" PRIX32,
		(uint32_t)SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);

	status = sss_key_object_allocate_handle(wallet_key,
					       SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET,
					       kSSS_KeyPart_Pair,
					       kSSS_CipherType_EC_NIST_K,
					       32, kKeyObject_Mode_Persistent);
	if (status != kStatus_SSS_Success) {
		se05x_demo_mark_fail_status(stats, "AllocateHandle(ETH_TESTNET_KEY)", status);
		return false;
	}

	status = sss_key_store_generate_key(&boot_ctx->ks, wallet_key, 256, NULL);
	if (status != kStatus_SSS_Success) {
		se05x_demo_mark_fail_status(stats, "GenerateKey(ETH_TESTNET_KEY)", status);
		return false;
	}

	printk("ETH_WALLET_KEY_OBJECT_ID=0x%08" PRIX32 "\n",
	       (uint32_t)SE05X_DEMO_OBJECT_ID_ETH_TESTNET_WALLET);
	printk("ETH_WALLET_KEY_CREATED=yes\n");
	se05x_demo_mark_pass(stats, "GenerateKey(ETH_TESTNET_KEY)");
	return true;
}
					   ...
					   const eth_legacy_tx_t *tx)
{
	...
	if (!eth_prepare_persistent_wallet_key(stats, boot_ctx, se_session, &wallet_key)) {
		return false;
	}
...
static sss_status_t run_eth_testnet_wallet_once(ex_sss_boot_ctx_t *boot_ctx,
					     const eth_legacy_tx_t *tx)
{
	...
	se05x_demo_stats_init(&stats, "ETH_TESTNET_WALLET");
	LOG_INF("ETH_TESTNET_WALLET started: legacy tx RLP + Keccak + SE05x secp256k1 sign");
...
static sss_status_t run_eth_testnet_address_once(ex_sss_boot_ctx_t *boot_ctx)
{
	se05x_demo_stats_t stats;
	sss_se05x_session_t *session = (sss_se05x_session_t *)&boot_ctx->session;
	pSe05xSession_t se_session = &session->s_ctx;
	...
	se05x_demo_stats_init(&stats, "ETH_TESTNET_ADDRESS");
	LOG_INF("ETH_TESTNET_ADDRESS started: create/reuse persistent secp256k1 wallet key");
	LOG_WRN("This command may write SE05x NVM if curve or wallet key is missing");

	if (eth_ensure_secp256k1_curve(&stats, se_session) &&
	    eth_prepare_persistent_wallet_key(&stats, boot_ctx, se_session, &wallet_key)) {
		(void)eth_export_public_key_and_address(&stats, boot_ctx, &wallet_key,
							public_point, from_address);
	}
...
static sss_status_t run_eth_testnet_delete_wallet_key(ex_sss_boot_ctx_t *boot_ctx)
{
	se05x_demo_stats_t stats;
	sss_se05x_session_t *session = (sss_se05x_session_t *)&boot_ctx->session;
	pSe05xSession_t se_session = &session->s_ctx;

	se05x_demo_stats_init(&stats, "ETH_TESTNET_DELETE_KEY");
	(void)eth_delete_wallet_key_if_confirmed(&stats, se_session);
	se05x_demo_log_summary(&stats);
	return se05x_demo_stats_result(&stats);
}

static sss_status_t run_eth_testnet_wallet(ex_sss_boot_ctx_t *boot_ctx)
{
	...
	while (1) {
		...
		if (strlen(line) == 4U &&
			...
			case 'H':
				...
				break;
			case 'A':
				printk("\nCMD AT+A -> Create/reuse wallet key and print address\n");
				last_status = run_eth_testnet_address_once(boot_ctx);
				break;
			case 'P':
			...
			case 'S':
				...
				last_status = run_eth_testnet_wallet_once(boot_ctx, &g_eth_tx);
				break;
			case 'Q':
				printk("\nCMD AT+Q -> Quit Demo 11\n");
				return last_status;
			default:
		...
		} else if (strcmp(line, "AT+X=DELETE_TESTNET_KEY") == 0 ||
			   strcmp(line, "at+x=delete_testnet_key") == 0) {
			printk("\nCMD AT+X=DELETE_TESTNET_KEY -> Delete persistent wallet key\n");
			last_status = run_eth_testnet_delete_wallet_key(boot_ctx);
		} else {
			printk("\nUnknown command. Type AT+H for help.\n");
		}
	}
...
const se05x_demo_t g_se05x_demo_eth_testnet_wallet = {
	.id = SE05X_DEMO_ETH_TESTNET_WALLET,
	.name = "eth_testnet_wallet",
	.when_to_use = "Create a stable SE05x-backed Sepolia wallet address and sign real testnet tx fields.",
	.flow = "Reuse/create persistent secp256k1 key, build RLP, Keccak it, sign in SE05x, print raw tx candidates.",
	.expected_output = "Stable ETH address, signing hash, DER signature, r/s, raw tx candidates, fail=0.",
	.se_features = "secp256k1 curve, EC_NIST_K persistent key, public key export, ECDSA sign/verify.",
	.run = run_eth_testnet_wallet,
};
