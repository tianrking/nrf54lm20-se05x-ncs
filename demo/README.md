...
推荐优先使用串口工具的 **text/文本发送模式**。例如要执行命令 `3`，应该发送字符 `3`，也就是 ASCII `0x33`。固件收到后会打印：

```text
RX text command '3' (raw=0x33) -> Se05x_API_GetRandom(16)
```

如果串口工具打开了 **hex encoded/十六进制发送模式**，输入 `03` 实际发送的是单字节 `0x03`，不是字符 `3`。当前 Demo 00 为了现场调试方便，已经兼容 `0x00..0x09`，所以发送 hex `03` 也会被当成命令 `3`。

```text
RX raw byte 0x03 normalized to command '3' -> Se05x_API_GetRandom(16)
```
...
se05x-safe-api> 1
RX text command '1' (raw=0x31) -> Se05x_API_GetVersion
OK   GetVersion len=7
...
RX text command '3' (raw=0x33) -> Se05x_API_GetRandom(16)
OK   GetRandom len=16
     Meaning: 16 raw random bytes generated inside SE05x; displayed as hex.
     Random len=16 hex=AC 87 41 A7 ED 5D B4 BB 82 0D 4C B8 94 BD 85 B2
se05x-safe-api> e
RX text command 'e' (raw=0x65) -> Se05x_API_ReadIDList
